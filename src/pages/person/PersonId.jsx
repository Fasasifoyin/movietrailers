...
import {ScrollButtons, Imagebox, Pagelayout} from "../../component";
...
const PersonId = () => {
  ...
  return (
    <Pagelayout src={`https://image.tmdb.org/t/p/original/${profile_path}`}
    error={error}>
      ...
      <div className="mt-4">
        ...
        <div className="d-flex scrollbody" style={{overflowX:'scroll', overflowY:`hidden`, width:'100%'}} ref={scrollRef}>
            {profiles.map((im, index) => (
              <div key={index} className='me-3'>
                  <Image src={`https://image.tmdb.org/t/p/w500/${im.file_path}`} className='rounded-3' loading="lazy" style={{width:'180px', height:'200px'}} onClick={() =>{
        ...
        {showPicModal && <Imagebox setShowPicModal={setShowPicModal} setIndex={setIndex} index={index} profiles={profiles}/>}
        </div>
      </div>
      <div className="mt-4">
      <h1 className="text-white fs-5 mt-3 mb-3">Recommendations</h1>
          {cast.length > 0 ? (
            ...
            <h1 className="text-secondary fs-6 mt-3 mb-3">No movie credits for {name}  at the moment</h1>
          )}
      </div>
    </Pagelayout>
  )
