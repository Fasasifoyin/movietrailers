...
import { Mediacard, PagelayoutId } from "../../component";
import { AiFillStar } from "react-icons/ai";
import useScroll from "../../hooks/useScroll";
import {ScrollButtons, Videobox, Imagebox} from "../../component";
    ...
    >
      <div className="d-md-flex gap-4">
        ...
        <div>
          ...
          <div className="text-white ">
            ...
            <p>{crew[0].job} - {crew[0].name}</p>
            <h1 className="fs-5">Genres</h1>
            <div className="d-flex flex-wrap  gap-2">
                {genres.map(genre => (
                  <p key={genre.id} className='small rounded-3 p-2 text-dark fw-bold' style={{backgroundColor: 'orange'}}>
                      {genre.name}
                  </p>
                ))}
            </div>
      ...
      <div className="mt-4 ">
         ...
         <div style={{position: 'relative'}}>
             <div className="d-flex scrollbody" style={{overflowX:'scroll', overflowY:`hidden`, width:'100%'}} ref={scrollRef}>
                  {results.map((video, index) => (
                    <div key={index} className='d-flex flex-column justify-content-between text-white me-3'>
                       <div className="p-1 rounded-3 text-center shadow trailerVid" onClick={() =>{
                        setShowModal(true)
                        setIndex(index)
                        }
                        } >
                           <p className="text-white small fw-bold mt-1 mb-0">{video.name.slice(0,30)}</p>
             ...
             {results.length > 5 ? (
              ...
              {showModal && <Videobox setShowModal={setShowModal} results={results} index={index} setIndex={setIndex}/>}
         </div>
         ) : (
          <h1 className="text-secondary fs-6 mt-3 mb-3">No videos for {title} at the moment</h1>
         )}
      </div>
      <div>
        ...
        {backdrops.length > 0 ? (
        <div style={{position:`relative`}}>
             <div className="d-flex scrollbody" style={{overflowX:'scroll', overflowY:'hidden', width:'100%'}} ref={scrollRefB}>
                  {backdrops.slice(0, 25).map((image, index) => (
                    <div key={index} className='me-3' onClick={() =>{
                      setShowPicModal(true)
                      setIndex(index)
                      }
                      }> 
                      <Image src={`https://image.tmdb.org/t/p/w500/${image.file_path}`}
                    className='rounded-3'
                    style={{
                      width: '270px',
                      height: '180px',
             ...
             {showPicModal && (
              <Imagebox setShowPicModal={setShowPicModal} index={index} setIndex={setIndex} backdrops={backdrops}/>
             )}
        </div> ) : (
         <h1 className="text-secondary fs-6 mt-3 mb-3">No images for {title} at the moment</h1>
      )}
      </div>
      <div className="mt-4">
            <h1 className="text-white fs-5 mt-3 mb-3">Cast</h1>
            <div style={{position: 'relative'}}>
            <div className="d-flex scrollbody" style={{overflowX:'scroll', overflowY:`hidden`, width:'100%'}} ref={scrollRefC}>
      ...
      <h1 className="text-white fs-5 mt-3 mb-3">Recommendations</h1>
          {resultsB.length > 0 ? (
            ...
            <h1 className="text-secondary fs-6 mt-3 mb-3">No recommendations for {title} at the moment</h1>
          )}
      </div>
    </PagelayoutId>
  );
